...
namespace TaskyJ.DataObjects
{
    ...
    public sealed class DynamicQuery
    {
        ...
        /// </returns>
        public static string GetInsertQuery(string tableName, string idName, dynamic item)
        {
            PropertyInfo[] props = item.GetType().GetProperties();
            string[] columns = props.RemoveUnneededProperties(idName).Select(p => p.Name).ToArray();

            return string.Format("INSERT INTO {0} ({1}) VALUES (@{2})", // "OUTPUT inserted.ID" not working with SQLite // "OUTPUT inserted." + idName + " VALUES (@{2})",
                                 tableName,
                                 string.Join(",", columns),
                                 string.Join(",@", columns));
        }
        ...
        /// The Sql query based on the item properties.
        /// </returns>
        public static string GetUpdateQuery(string tableName, string idName, dynamic item)
        {
            PropertyInfo[] props = item.GetType().GetProperties();
            string[] columns = props.RemoveUnneededProperties(idName).Select(p => p.Name).ToArray();
            ...
            return string.Format("UPDATE {0} SET {1} WHERE " + idName + "=@ID", tableName, string.Join(",", parameters));
        }
                                     ...
                                     ref List<QueryParameter> queryProperties)
        {
            if (body.NodeType != ExpressionType.AndAlso && body.NodeType != ExpressionType.OrElse)
            {
                ...
                if (propertyValue.GetType().GetProperty("Value") != null)
                    queryProperties.Add(new QueryParameter(link, propertyName, propertyValue.Value, opr));
                else
                    queryProperties.Add(new QueryParameter(link, propertyName, Expression.Lambda((MemberExpression)((BinaryExpression)body).Right).Compile().DynamicInvoke(), opr));
            }
            else
            {
